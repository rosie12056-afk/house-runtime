...
- v0.3 has two independent Memory Adapter implementations and two clients under candidate conformance. Runtime also durably queues adapter writes, retries them after restart, and exposes delivery state.
- The public API remains unstable until request and response contracts live in Protocols, embedded-memory migration is tested, and cross-repository candidate CI passes.
