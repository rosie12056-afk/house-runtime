...
| v0.3 | Candidate work on `t4-portability`; release requires adapter integration and a second client | Asynchronous Memory Adapter API, two standalone adapter implementations, connector capability boundaries, Console API profile, migration tooling, and the release gate for optional Anchor and Console repositories | Proves that Runtime APIs are not tied to one database, UI, or private House instance | Adapter and client authors |
