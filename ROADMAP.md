...
| `v0.1.0-alpha.2` | After Protocols v0.2 RC and Toolkit v0.2 conformance | Scheduler leases, cancellation, timeout, fake-clock tests, confirmation challenges, Resignature execution, fuller Memory Port, audit events, and retry budgets | Makes long-running and risky work controllable, testable, and recoverable | Persistent-agent builders |
| v0.2 | After alpha.2 restart and time-boundary tests pass | Life Clock, sleep windows, tick opportunities, journal, dream, handoff, and Initiative research-to-delivery-to-feedback loops | Adds 24-hour continuity without turning autonomy into repetitive task reports or ungrounded memory | Single-user continuous-agent deployments |
| v0.3 | After a second Memory Adapter and a second client pass conformance | Stable Memory Adapter API, connector capability boundaries, Console API profile, migration tooling, and the release gate for optional Anchor and Console repositories | Proves that Runtime APIs are not tied to one database, UI, or private House instance | Adapter and client authors |
...
- v0.3 requires two independent Memory Adapter implementations and two clients before the public API is called stable.
