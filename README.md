...
Runtime v0.3.0-rc.2 contains a Memory Port candidate with two standalone adapters that use their own data files rather than RuntimeStore tables: `SQLiteMemoryAdapter` and `JsonFileMemoryAdapter`. Both expose an asynchronous API and pass the same Toolkit conformance suite. When an Instance supplies `memoryAdapter`, Runtime commits a durable Memory Operation before delivery, retries idempotently after restart, and exposes `pending`, `delivered`, or `failed` state. The released v0.2 line remains unchanged.

The release candidate also contains a transport-neutral `RuntimeService`, a direct module client, and a JSON serialization client. RC2 adds read-only Run listing plus Evidence and Initiative readback for operational clients. Sensitive methods fail closed without a host authorization callback. Authentication context is attached by the host transport and is never accepted from request parameters.
